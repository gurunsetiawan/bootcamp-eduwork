...
<body>
            ...
            <div class="col-lg-6">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="index.php"><?= htmlspecialchars($produk['kategori']) ?></a></li>
                ...
                <div class="mb-3">
                    <span class="badge bg-primary stock-badge"><?= htmlspecialchars($produk['kategori']) ?></span>
                    <span class="badge bg-<?= $produk['stok'] > 10 ? 'success' : ($produk['stok'] > 0 ? 'warning' : 'danger') ?> stock-badge ms-2">
                        Stok: <?= $produk['stok'] ?>
                    </span>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <p class="mb-0 fw-bold">#<?= $produk['id'] ?></p>
                            </div>
                            <div class="col-6">
                                <small class="text-muted">Ditambahkan</small>
                                <p class="mb-0 fw-bold"><?= date('d M Y', strtotime($produk['created_at'])) ?></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadRelatedProducts() {
            try {
                const response = await fetch(`api/get_products.php?kategori=${encodeURIComponent('<?= $produk['kategori'] ?>')}&urutan=default`);
                const result = await response.json();
                
                if (result.status === 'success') {
                    const relatedProducts = result.data.filter(p => p.id !== <?= $produk['id'] ?>).slice(0, 4);
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-6">
                                <div class="card h-100 shadow-sm">
                                    <img src="${produk.gambar}" class="card-img-top" alt="${produk.nama}" 
                                         style="height: 200px; object-fit: cover;" onerror="this.src='assets/placeholder.jpg'">
